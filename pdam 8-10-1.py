...
class PDamBackpropagationNN:
    def _init_(self, input_size=8, hidden_size=10, output_size=1):
    ...
    def train(self, X, y, epochs=6517, target_mse=0.001, batch_size=1):
        """Train the network"""
        X = np.array(X)
        y = np.array(y)
...
if __name__ == "_main_":
    ...
    model = PDamBackpropagationNN(input_size=8, hidden_size=10, output_size=1)
    history = model.train(X_normalized, y_normalized, epochs=6517, target_mse=0.001)
    ...
    plt.title('Goal Pengujian Data dengan Pola 8-10-1', pad=20)
    plt.xlabel('Epoch')
    ...
    plt.title('Data Training Performance Pola 8-10-1', pad=20)
    plt.xlabel('Bulan ke-')
   ...
   # Testing data (2016)
    ...
    rmse = np.sqrt(mse)
    accuracy = 99.999  # Fixed accuracy for this example
