...
class PDamBackpropagationNN:
    def __init__(self, input_size=8, hidden_size=9, output_size=1):
        ...
        self.input_size = input_size #Menentukan jumlah fitur input (8 kolom dari data, tanpa kolom target)
        self.hidden_size = hidden_size #Jumlah neuron pada lapisan tersembunyi.
        self.output_size = output_size #Jumlah neuron pada lapisan output (hanya satu karena targetnya adalah satu nilai).
        
        # Initialize learning parameters
        self.learning_rate = 0.5 #Kecepatan pembelajaran
        self.momentum = 0.9 #Faktor momentum untuk mempercepat konvergensi
        
        # Initialize weights with small random values
        np.random.seed(42)  #Angka 42 adalah berasal dari buku The Hitchhiker's Guide to the Galaxy oleh Douglas Adams "jawaban untuk kehidupan, alam semesta, dan segala sesuatu."
    ...
    #Fungsi untuk menormalisasi data agar berada dalam rentang tertentu (0.1 hingga 0.8).
    
    def forward(self, X):
        """Forward pass through the network"""
        if len(X.shape) == 1:
            X = X.reshape(1, -1)
        #Fungsi untuk menjalankan forward pass di jaringan saraf
    ...
    def backward(self, X, y, output):
        ...
        if len(y.shape) == 1:
            y = y.reshape(-1, 1)
        #Fungsi ini digunakan untuk melakukan backpropagation. Ini adalah bagian penting dari pelatihan jaringan saraf, di mana kita menghitung kesalahan dan 
        #memperbarui bobot dengan menggunakan turunan fungsi aktivasi (sigmoid) dan menggunakan momentum untuk mempercepat konvergensi.
    ...
    def train(self, X, y, epochs=4595, target_mse=0.001, batch_size=1):
        ...
        X = np.array(X)
        y = np.array(y)
        #Fungsi untuk melatih model menggunakan data X dan y selama sejumlah epoch.
    ...
    #Fungsi ini digunakan untuk menghasilkan prediksi setelah model dilatih. Data input diproses melalui jaringan saraf yang telah terlatih, dan hasilnya adalah prediksi.
